...
const SMARTAI_GLOBAL_DIR = path.join(__dirname, "..", ".continue");
if (fs.existsSync(SMARTAI_GLOBAL_DIR)) {
  fs.rmSync(SMARTAI_GLOBAL_DIR, { recursive: true, force: true });
}
fs.mkdirSync(SMARTAI_GLOBAL_DIR);
...
describe("Test Suite", () => {
  ...
  beforeAll(async () => {
    ...
    const binaryPath = path.join(binaryDir, `smartai-binary${exe}`);
    const expectedItems = [
      `smartai-binary${exe}`,
      `rg${exe}`,
      "index.node",
    ...
    if (USE_TCP) {
      ...
      try {
        subprocess = spawn(binaryPath, {
          env: { ...process.env, SMARTAI_GLOBAL_DIR },
        });
  ...
  it("should create .continue directory at the specified location with expected files", async () => {
    expect(fs.existsSync(SMARTAI_GLOBAL_DIR)).toBe(true);
    ...
    const missingFiles = expectedFiles.filter((file) => {
      const filePath = path.join(SMARTAI_GLOBAL_DIR, file);
