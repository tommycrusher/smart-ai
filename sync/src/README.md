...
The first time, a Merkle tree of the codebase folder is constructed, ignoring any files in .gitignore or .smartaiignore. Every file found will be returned as needing to be computed added to the index.
...
Several files are stored and updated on disk in the ~/.smart-ai/index folder to keep track of indexed files:

- `~/.smart-ai/index/tags/<dir>/<branch>/<provider_id>/merkle_tree` - the last computed Merkle tree of the codebase for a given tag
- `~/.smart-ai/index/tags/<dir>/<branch>/<provider_id>/.last_sync` - the last time the tag was synced
- The index cache contains a list of hashes that have already been computed both in general and per tag. These are always kept in sync.
  - `~/.smart-ai/index/.index_cache` - contains the global cache (flat file of hashes)
  - `~/.smart-ai/index/tags/<dir>/<branch>/<provider_id>/.index_cache` - contains the tag-specific cache (flat file of hashes)
  - `~/.smart-ai/index/rev_tags` - contains a mapping from hash to tags that the hash is currently indexed for. This is a directory of files, where each file is prefixed with the first 2 characters of the hash. The file is a JSON mapping from hash to list of tags.
...
- Only handles local files, so is not currently being used in situations where the Smart AI server is on a different machine from the IDE or the workspace (Remote SSH, WSL, or a Smart AI server being run for a team).
- Currently not using stat to check for recent changes to files, is instaed re-calculating the entire Merkle tree on every IDE reload. This is fine for now since it only takes 0.2 seconds on the Smart AI codebase, but is a quick improvement we can make later.
