    ...
    for entry in fs::read_dir(workspace_dir)? {
        ...
        if path.is_dir() {
            ...
            match path.file_name().and_then(|name| name.to_str()) {
                Some(file_name) if file_name.ends_with(".gitignore") || file_name.ends_with(".smartaiignore") => {
...
mod tests {
    ...
    fn test_local_find_gitignores() -> io::Result<()> {
        ...
        // Create some directories and .gitignore/.smartaiignore files
        let dir_structure = [
            ("dir1", Some(".gitignore"), "node_modules/"),
            ("dir2", Some(".smartaiignore"), "target/"),
            ("dir3", None, ""),
        ];
