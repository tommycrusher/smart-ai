...
description: "Comprehensive guide to resolving common issues with Smart AI, including logging, keyboard shortcuts, networking problems, model capabilities, and extension configuration troubleshooting"
---
...
#### Prompt Logs (Smart AI Console)

To view prompt logs/analytics, you can enable the Smart AI Console.

1. Open VS Code settings (`cmd/ctrl` + `,`)
2. Search for the setting "Smart AI: Enable Console" and enable it
3. Reload the window
4. Open the Smart AI Console by using the command palette (`cmd/ctrl` + `shift` + `P`) and searching for "Smart AI: Focus on Smart AI Console View"

![Smart AI Console](/images/images/continue-console-d387a10c2918c117c6c253a3b5f18c22.png)

### JetBrains

Open `~/.smart-ai/logs/core.log` to view the logs for the Smart AI plugin. The most recent logs are found at the bottom of the file.
...
We are constantly making fixes and improvements to Smart AI, but the latest changes remain in a "pre-release" version for roughly a week so that we can test their stability. If you are experiencing issues, you can try the pre-release by going to the Smart AI extension page in VS Code and selecting "Switch to Pre-Release" as shown below.
...
For VS Code, All versions are hosted on the Open VSX Registry [here](https://open-vsx.org/extension/Smart AI/continue). Once you've downloaded the extension, which will be a .vsix file, you can install it manually by following the instructions [here](https://code.visualstudio.com/docs/editor/extension-gallery#_install-from-a-vsix).
...
If your keyboard shortcuts are not resolving, you may have other commands that are taking precedence over the Smart AI shortcuts. You can see if this is the case, and change your shortcut mappings, in the configuration of your IDE.
...
This issue typically affects macOS users with large development environments and is being tracked in [#7870](https://github.com/tommycrusher/smart-ai/issues/7870) and [#6699](https://github.com/tommycrusher/smart-ai/issues/6699).

## SSL certificate errors

If Smart AI can reach your model endpoint but cannot verify its TLS certificate chain, you may see `fetch failed` alongside errors such as:
...
1. Reproduce the error in the Smart AI logs so you can confirm the exact hostname that failed.
...
5. Verify the fix with curl before retrying in Smart AI:
