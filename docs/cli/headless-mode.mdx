...
Set `SMARTAI_API_KEY` instead of running `cn login`:
...
Get an API key from [Smart AI → Settings → API Keys](https://smart-ai.dev/settings/api-keys). Store it as a secret in your CI provider.
