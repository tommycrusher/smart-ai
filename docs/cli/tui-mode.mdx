...
Run `cn` to start an interactive session. You get a prompt where you can type messages, reference files with `@`, and use slash commands. `cn` uses the same underlying agent as the Smart AI IDE extensions.
...
| `/update` | Update the Smart AI CLI |
...
When you're connected to a remote environment, Smart AI also adds remote-only slash commands such as `/diff` and `/apply`.
...
- **Smart AI** — approve this call
- **Smart AI + don't ask again** — approve and save a policy rule to `~/.smart-ai/permissions.yaml`
