...
Smart AI CLI (`cn`) is a terminal-based coding agent. It can edit files, run commands, and work through multi-step tasks — the same agent that powers the Smart AI IDE extensions, running in your terminal.
...
- A [Smart AI](https://smart-ai.dev) account, or an Anthropic API key.
...
On first launch you'll be asked to log in with [Smart AI](https://smart-ai.dev) or enter an Anthropic API key. After that, you're in a session and can start typing.

## Authentication

### Log in with Smart AI

```bash
cn login
```

This opens your browser to authenticate with [Smart AI](https://smart-ai.dev). Once authenticated, `cn` can use your configured assistants, models, and MCP servers from the platform.

### API key (headless / CI)

For automation environments where there's no browser, set the `SMARTAI_API_KEY` environment variable:

```bash
export SMARTAI_API_KEY=your-key-here
cn -p "your prompt"
```

Get an API key from [Smart AI → Settings → API Keys](https://smart-ai.dev/settings/api-keys).

### Local API key

If you don't want to use Smart AI, you can use an Anthropic API key directly. On first launch, `cn` will prompt you to choose between logging in with Smart AI or entering an Anthropic API key.
