...
3. **Default** — your first assistant from [Smart AI](https://smart-ai.dev), or `~/.smart-ai/config.yaml` if you're not logged in
...
If you're not logged in to Smart AI, `cn` looks for `~/.smart-ai/config.yaml`. This file uses the same format as the IDE extensions — see [config.yaml reference](/customize/deep-dives/configuration) for the full schema.
...
If you belong to multiple organizations on Smart AI, use `--org` to specify which one:
...
Store sensitive values (API keys, tokens) in [Smart AI → Settings → Secrets](https://smart-ai.dev/settings/secrets). Reference them in config with:
