...
description: "Comprehensive guide to the config.yaml format used by Smart AI.dev for building custom coding agents. Learn how to define models, context providers, rules, prompts, and more using YAML configuration."
---

## Introduction

Smart AI Agents are defined using the `config.yaml` specification.
...
Learn how to work with Smart AI's configuration system, including using hub models, rules, and tools, creating local configurations, and organizing your setup.
</Card>
...
- `capabilities`: Array of strings denoting model capabilities, which will overwrite Smart AI's autodetection based on provider and model. See the [Model Capabilities guide](/customize/deep-dives/model-capabilities) for detailed information. Supported capabilities include:
  ...
  Smart AI automatically detects these capabilities for most models, but you can override this when using custom deployments or if autodetection isn't working correctly.
...
schema: v1
rules:
  - uses: sanity/sanity-opinionated # rules file stored on Smart AI Mission Control
...
schema: v1
prompts:
  - uses: supabase/create-functions # prompts file stored on Smart AI Mission Control 
...
docs:
  - name: Smart AI
    startUrl: https://docs.smart-ai.dev/intro
    favicon: https://docs.smart-ai.dev/favicon.ico
```
...
The [Model Context Protocol](https://modelcontextprotocol.io/introduction) is a standard proposed by Anthropic to unify prompts, context, and tool use. Smart AI supports any MCP server with the MCP context provider.
...
data:
  - name: Local Data Bank
    destination: file:///Users/dallin/Documents/code/tommycrusher/smart-ai-extras/external-data
    schema: 0.2.0
    level: all
