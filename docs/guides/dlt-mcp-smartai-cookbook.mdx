---
title: "Building Data Pipelines with dlt MCP and Smart AI"
...
sidebarTitle: "dlt Data Pipelines with Smart AI"
---
...
<Card title="What You'll Build" icon="database">
  An AI-powered data pipeline development system that uses Smart AI's AI agent with dlt
...
<Steps>
<Step title="Install Smart AI CLI">
    <CLIInstall />
  </Step>
...
<Step title="Install dlt">
 ...
 <Warning>
  To use agents in headless mode, you need a [Smart AI API key](https://smart-ai.dev/settings/api-keys).
  </Warning>

## dlt MCP Workflow Options

<Card title="🚀 Fastest Path to Success" icon="zap">
  Skip the manual setup and use our pre-built [dlt Agent](https://smart-ai.dev/continuedev/dlt-agent) that includes
...
<Steps>
  <Step title="Create a New Agent via the Smart AI Mission Control">
    Go to the [Smart AI Mission Control](https://smart-ai.dev) and [create a new agent](https://smart-ai.dev/agents/new).
    </Step>

  <Step title="Connect dlt MCP via Smart AI Mission Control">
    Visit the [dlt MCP on Smart AI Mission Control](https://smart-ai.dev/dlthub/dlt-mcp) and click **Install** to add it to the agent you created in the step above.
    ...
    <Tip>
      **Alternative installation methods:**
      ...
      2. **Manual configuration**: Add the MCP to your `~/.smart-ai/config.json` under the `mcpServers` section

      Once installed, dlt MCP tools become available to your Smart AI agent for all prompts.
    </Tip>
...
<Accordion title="Agent Requirements">
    To use the pre-built [dlt Agent](https://smart-ai.dev/continuedev/dlt-agent), you need either:
      - **Smart AI CLI Pro Plan** with the models add-on, OR
      - **Your own API keys** added to Smart AI Mission Control secrets (same as manual setup)
...
<Card title="Understanding the Difference" icon="info-circle">
  ...
  **[dlt+ MCP](https://smart-ai.dev/dlthub/dlt-plus-mcp)** extends these capabilities with cloud-based features for production deployments:
  ...
  For local development and getting started, **[dlt MCP](https://smart-ai.dev/dlthub/dlt-mcp)** is the right choice. Consider **[dlt+ MCP](https://smart-ai.dev/dlthub/dlt-plus-mcp)** when you need production deployment features and team collaboration.
</Card>
...
Now you can use natural language prompts to develop and debug your dlt pipelines. The Smart AI agent automatically calls the appropriate dlt MCP tools.

<Info>
You can add prompts to your agent's configuration for easy access in future sessions. Go to your agent in the [Smart AI Mission Control](https://smart-ai.dev), click **Edit**, and add prompts under the **Prompts** section.
</Info>

<Info>
  **Where to run these workflows:**
  - **IDE Extensions**: Use Smart AI in VS Code, JetBrains, or other supported IDEs
...
## Continuous Data Pipelines with GitHub Actions
  ...
  This example demonstrates a **Continuous AI workflow** where data pipeline validation runs automatically in your CI/CD pipeline in headless mode using the [dlt Assistant agent](https://smart-ai.dev/dlthub/dlt-assistant). Consider [remixing this agent](/guides/understanding-configs#how-to-get-started-with-hub-configs) to add your organization's specific validation rules.

### Add GitHub Secrets

Navigate to **Repository Settings → Secrets and variables → Actions** and add:

- `SMARTAI_API_KEY`: Your Smart AI API key from [smart-ai.dev/settings/api-keys](https://smart-ai.dev/settings/api-keys)
- Any required database credentials for your destination

<Info>
      The workflow uses the pre-built [dlt Agent](https://smart-ai.dev/continuedev/dlt-agent) with `--agent continuedev/dlt-agent`. This agent comes pre-configured with the dlt MCP and optimized rules for pipeline operations. You can [remix this agent](/guides/understanding-configs#how-to-get-started-with-hub-configs) to customize the validation rules and prompts for your specific pipeline requirements.
</Info>

### Create Workflow File

This workflow automatically validates your dlt data pipelines on pull requests using the Smart AI CLI in [headless mode](/cli/headless-mode). It inspects pipeline schemas, checks for errors, and posts a summary report as a PR comment. The workflow can also be triggered manually via `workflow_dispatch`.
...
Implement automated pipeline quality checks using Smart AI's rule system. See the [Rules deep dive](/customize/deep-dives/rules) for authoring tips.
...
<Check>
  **Verification Steps:**
  - dlt MCP is installed via [Smart AI Mission Control](https://smart-ai.dev/dlthub/dlt-mcp)
...
<Card title="Continuous AI" icon="rocket">
  ...
  AI](https://blog.smart-ai.dev/what-is-continuous-ai-a-developers-guide/)** -
...
<CardGroup cols={2}>
  ...
  <Card title="Smart AI Mission Control" icon="plug" href="https://smart-ai.dev">
  ...
  >
    Learn about AI agents, MCP, and Smart AI integration
  </Card>
  <Card
    title="MCP Concepts"
    icon="puzzle-piece"
