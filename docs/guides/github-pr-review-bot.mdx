---
title: "Code Review Bot with Smart AI and GitHub Actions"
description: "Set up automated, context-aware pull request reviews using Smart AI CLI in GitHub Actions - privacy-first with custom rules"
...
<CardGroup cols={3}>
  <Card title="Privacy-First" icon="shield">
    All logs and processing happen in your runner: Smart AI CLI runs in GitHub Actions → code to your LLM provider (OpenAI, Anthropic, etc.). No hosted Smart AI service reads your code.

  </Card>

  <Card title="Customizable" icon="sliders">
    Define team-specific rules in `.smart-ai/rules/` that automatically apply to every pull request.

  </Card>

  <Card title="Context Awareness" icon="robot">
    Leverage Smart AI's AI agent for intelligent, context-aware reviews with full control over your configuration.
...
- A Smart AI API key from [smart-ai.dev/settings/api-keys](https://smart-ai.dev/settings/api-keys)
- Smart AI assistant configured for code reviews (or use our recommended default)

<Info>
  **Want to customize the review bot?**
  
  You can remix the default review bot configuration at [smart-ai.dev/continuedev/review-bot](https://smart-ai.dev/continuedev/review-bot) to create your own personalized version with custom prompts, rules, and behaviors.
</Info>
...
jobs:
  review:
    ...
    steps:
      ...
      - name: Run Smart AI Review
        env:
          SMARTAI_API_KEY: ${{ secrets.SMARTAI_API_KEY }}
          GH_TOKEN: ${{ steps.app-token.outputs.token || github.token }}
        run: |
          # Check if custom rules exist
          if [ -d ".smart-ai/rules" ]; then
            echo "📋 Found custom rules in .smart-ai/rules/"
            RULES_CONTEXT="Apply the custom rules found in .smart-ai/rules/ directory."
          else
          ...
          # Run Smart AI CLI in headless mode
      ...
      - name: Post Review Comment
        ...
        run: |
          ...
          ---
          *Powered by [Smart AI](https://smart-ai.dev) • Need a focused review? Comment `@review-bot check for [specific concern]`*
          EOF
...
Define your team's standards in `.smart-ai/rules/`:

<Tabs>
  <Tab title="Security Rule">
    
 Create `.smart-ai/rules/security.md`:
...
```
  ...
  <Tab title="TypeScript Standards">
    Create `.smart-ai/rules/typescript.md`:
...
Create `.smart-ai/rules/testing.md`:
...
Create `.smart-ai/rules/python.md`:
...
3. **Load Custom Rules** - Reads your team's rules from `.smart-ai/rules/`
4. **Get Pull Request Diff** - Fetches the diff and list of changed files
5. **Smart AI CLI Analyzes Code** - AI agent reviews the code with your rules
...
<Tabs>
  <Tab title="Use Your Own Smart AI Config">
  ...
  - name: Run Smart AI Review
    env:
      SMARTAI_API_KEY: ${{ secrets.SMARTAI_API_KEY }}
      SMARTAI_ORG: your-org-name      # Add your org
      SMARTAI_CONFIG: username/config-name  # Add your config
    run: |
      cn --config $SMARTAI_ORG/$SMARTAI_CONFIG \
        -p "$PROMPT" \
        --auto > review_output.md
  ```

  Store `SMARTAI_ORG` and `SMARTAI_CONFIG` as repository variables for easy updates.
</Tab>
...
## Troubleshooting
<AccordionGroup>
<Accordion title="Smart AI CLI not found">
- The workflow installs the CLI automatically, but ensure Node.js 20+ is available
- Check the "Install Smart AI CLI" step logs for errors
</Accordion>
<Accordion title="Authentication failed">
- Verify your `SMARTAI_API_KEY` is valid
- Check that GitHub token has required permissions
</Accordion>
<Accordion title="No review generated">
- Verify your Smart AI config is accessible
- Check Smart AI CLI logs in the workflow run
...
>
> ### Recommendations
> 1. Move `secretKey` to environment variables (see `.smart-ai/rules/security.md`)
...
>
> ---
> *Powered by [Smart AI](https://smart-ai.dev) • Need a focused review? Comment `@review-bot check for security`*
...
5. **Create team config** - Set up a shared Smart AI config for consistent reviews
...
<CardGroup cols={2}>
  ...
  <Card title="Smart AI CLI Guide" icon="terminal" href="/guides/cli">
    Learn more about Smart AI CLI capabilities
  </Card>
  <Card title="Smart AI Mission Control" icon="globe" href="https://smart-ai.dev">
...
- Share improvements as [GitHub discussions](https://github.com/tommycrusher/smart-ai/discussions)
