---
title: "Developer & Team Workflows with Notion + Smart AI CLI"
description: "Use Smart AI CLI with Notion to generate docs, manage tasks, and automate project workflows – all through natural-language prompts."
sidebarTitle: "Notion with Smart AI"
---
...
- Use natural language to connect to the Notion API directly with Smart AI CLI for powerful automation
...
- [Smart AI CLI](https://docs.smart-ai.dev/cli/quickstart) installed
- A [Notion workspace](https://notion.so) with Editor (or higher) access
- Node.js 18+ installed locally
- [Smart AI account](https://smart-ai.dev) with **Hub access**

<Warning>

  **Agent usage requires credits** – create a Smart AI API key at
  [smart-ai.dev/settings/api-keys](https://smart-ai.dev/settings/api-keys)
...
<Steps>
  ...
  <Step title="Create Notion Integration & Get API Key">
    ...
    2. Click **+ New integration** → give it a name (e.g. "Smart AI Integration")
...
## Running Smart AI CLI with Notion API

<Card title="🚀 Choose Your Interface" icon="zap">

  Smart AI CLI offers two powerful modes for Notion automation:
...
<Accordion title="API Connection Notes">
  ...
  - Environment variable `NOTION_API_KEY` must be set before running Smart AI CLI
  - Smart AI automatically uses the API key to authenticate with Notion
  - No need for manual curl commands - just reference "the API key stored in this session"
  - For complex workflows, Smart AI maintains the API connection throughout
...
Working example that demonstrates the power of Smart AI with Notion API:
...
<CardGroup cols={1}>
  <Card title="Notion + GitHub" icon="github">
    ...
    <Info>
      ...
      Requires GitHub repository access. To add GitHub access, update your [integration settings](https://smart-ai.dev/settings/integrations).
...
<Accordion title="Common Issues and Solutions">
  ...
  **Connection Issues:**
  - Verify Smart AI CLI has internet access
