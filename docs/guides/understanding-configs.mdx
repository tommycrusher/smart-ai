...
description: "Learn how to choose between cloud-managed Hub and local configuration for AI development assistance in Smart AI, including setup, management, and best practices for each approach"
---
...
Every developer has unique needs when it comes to AI assistance. Some prefer the convenience of cloud-managed configurations, while others need the control and privacy of local setups. Smart AI offers both paths, and this guide will help you choose the right one for your workflow.

## What Are the Two Paths to AI Assistance?

Smart AI provides two distinct ways to configure:

Think of Smart AI's configuration options like choosing between a managed service and self-hosting. Both get you to the same destination—powerful AI assistance in your IDE—but the journey and control level differ significantly.

### How to Access Your Configuration

Before we dive into the specifics, let's understand how to access your configuration:

1. Open the Smart AI Chat sidebar by pressing <kbd>cmd/ctrl</kbd> + <kbd>L</kbd> (VS Code) or <kbd>cmd/ctrl</kbd> + <kbd>J</kbd> (JetBrains)
...
Hub Configurations represent the "it just works" philosophy. When you [sign in to Smart AI Mission Control](https://auth.smart-ai.dev/), you gain access to a curated ecosystem of established configurations that sync seamlessly across all your development environments.

### Why Should You Choose Hub Configs?

**The Power of Simplicity**

- **Instant Setup**: Browse the [configuration marketplace](https://smart-ai.dev) and add any config to your account with a single click
...
1. **Select Your Config**: Click the config selector in your IDE's Smart AI panel
2. **Explore or Create**: Browse community configurations or craft your own specialized setup
3. **Secure Your Keys**: Add API keys as [User Secrets](https://smart-ai.dev/settings/secrets) in Mission Control—they're encrypted and never exposed
...
All Hub config management happens through [Mission Control](https://smart-ai.dev). For detailed customization, see our guide on [Editing a Config](/mission-control/configs/edit-a-config).

## What Are Local Configs: The Power User's Choice

Local configuration puts you in the driver's seat. Using a `config.yaml` file, you have complete control over every aspect of your Smart AI experience with all configuration stored directly on your machine.
...
- macOS/Linux: `~/.smart-ai/config.yaml`
...
When you edit your `config.yaml`, Smart AI provides intelligent autocomplete for all available options. Save the file, and Smart AI automatically reloads your configuration—no restart required.

The first time you use Smart AI, it generates a `config.yaml` with sensible defaults. From there, you can customize everything from model selection to context providers, slash commands, and more.
...
- Check YAML syntax (Smart AI will show errors)
- Ensure file permissions allow reading

**Autocomplete Not Working?**

- Update to the latest Smart AI version
