...
description: "Set up version-controlled agents in your repository and run them with Smart AI CLI or GitHub Actions"
---
...
<Card title="What You'll Build" icon="folder-open">
  ...
  - Runs agents with Smart AI CLI for local development
...
- [Smart AI CLI installed](/cli/quickstart)
...
<Steps>
  ...
  <Step title="Create your first agent">
    ...
    ```bash
    touch .smart-ai/agents/my-agent.md
    ```
  ...
  <Step title="Run the agent">
    ```bash
    cn -p --agent .smart-ai/agents/my-agent.md
    ```

    For interactive development (TUI mode):

    ```bash
    cn --agent .smart-ai/agents/my-agent.md
    ```
  </Step>
</Steps>
...
Smart AI provides a [reusable workflow](https://github.com/tommycrusher/smart-ai/blob/main/.github/workflows/smartai-agents.yml) that handles agent discovery, parallel execution, and GitHub Check reporting.
...
jobs:
  agents:
    uses: continuedev/continue/.github/workflows/smartai-agents.yml@main
    secrets:
      ANTHROPIC_API_KEY: ${{ secrets.ANTHROPIC_API_KEY }}
```

The reusable workflow automatically:
- Discovers all `.md` files in `.smart-ai/agents/`
...
jobs:
  agents:
    uses: continuedev/continue/.github/workflows/smartai-agents.yml@main
    with:
      agents-path: '.smart-ai/agents'  # Custom agents directory (optional)
    secrets:
      ANTHROPIC_API_KEY: ${{ secrets.ANTHROPIC_API_KEY }}
```
...
<Tabs>
  ...
  <Tab title="CLI Tool Permissions">
    ...
    cn --allow "Read()" --allow "Grep()" --exclude "Bash()" \
       --agent .smart-ai/agents/analysis-only.md
    ```
...
<AccordionGroup>
  ...
  <Accordion title="Agent runs but doesn't complete the task">
    ...
    - Enable verbose logging: `cn --verbose --agent .smart-ai/agents/my-agent.md`
    - Review logs with `cn --verbose` or check the Smart AI logs directory
  </Accordion>
  ...
  <Accordion title="Workflow doesn't discover agents">
    ...
    ```bash
    find .smart-ai/agents -name "*.md" -type f
    ```

    The workflow looks for `.md` files in `.smart-ai/agents/`.
  </Accordion>
</AccordionGroup>
...
<CardGroup cols={2}>
  ...
  <Card title="CLI Reference" icon="terminal" href="/cli/quickstart">
    Full Smart AI CLI documentation
  </Card>
