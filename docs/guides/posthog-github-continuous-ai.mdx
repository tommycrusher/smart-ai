...
sidebarTitle: "PostHog Analytics with Smart AI CLI"
---
...
<Card title="What You'll Build" icon="robot">
  A fully automated workflow that uses Smart AI CLI with the PostHog MCP to fetch analytics data, analyze user experience issues with AI, and automatically create GitHub
...
<Steps>
  ...
  <Step title="Add Required Secrets to Smart AI CLI">
    Smart AI CLI will securely store your API keys as secrets that can be referenced in prompts.
  </Step>
</Steps>

<Tip>
  Smart AI CLI handles the complex API interactions - you just need to provide
  the right prompts!
</Tip>

## Step 1: Set Up Your Credentials

First, you'll need to gather your PostHog and GitHub API credentials and add them as secrets in Smart AI CLI.
...
<Tabs>
  <Tab title="PostHog API Credentials">
    ...
    3. Name it "Smart AI CLI Session Analysis"
    ...
    8. You'll also need your POSTHOG_AUTH_HEADER value, which is simply `Bearer YOUR_API_KEY`
      ...
      <Info>
  **Smart AI Secrets**: The `POSTHOG_AUTH_HEADER` secret should be stored in
  Smart AI's secure secrets storage. This keeps your API key safe and the MCP
...
<Tab title= "Configure API Access in Smart AI CLI">
 
<Tip>
See the [secret types documentation](/mission-control/secrets/secret-types) for adding secrets
</Tip>

You only need to configure the PostHog MCP credential - it automatically handles project selection. To add environment variables to your Smart AI Mission Control account:

1.  Go to the [Smart AI Mission Control](https://smart-ai.dev)
...
<Tabs>
    ...
    **Perfect for:** Immediate results with optimized prompts and built-in debugging
    ...
    <Steps>
      <Step title="Add the Pre-Built Agent">
        Visit the [PostHog Continuous AI Agent](https://smart-ai.dev/continuedev/posthog-continuous-ai-agent) on Smart AI Mission Control and click **"Install Agent"** or run:
  ...
  <Steps>
    <Step title="Add PostHog MCP to Smart AI CLI">
      First, install the [PostHog MCP](https://smart-ai.dev/posthog/http-mcp) to the Smart AI CLI by
    ...
    <Step title="Add PostHog + GitHub Analysis Rules">
      Visit [PostHog GitHub Continuous AI Rules](https://smart-ai.dev/bekah-hawrot-weigel/posthog-github-continuous-ai-rules) and click "Use Rule" to add it to your Config.
      
      You could also
      1. Pass `--rule bekah-hawrot-weigel/posthog-github-continuous-ai-rules` to `cn` OR
      2. Copy the rules to your local `.smart-ai/rules` folder. See the [Rules Guide](/customize/deep-dives/rules#how-to-create-rules).
    </Step>
    <Step title="Create Your Custom Prompts">
      Use this prompt with Smart AI CLI to analyze PostHog data and create GitHub issues:
...
Here are practical examples of what you can build with PostHog MCP and Smart AI CLI:
...
Consider enhancing your workflow with these advanced Smart AI CLI prompts:
...
- Consider [GitHub MCP](https://smart-ai.dev/github/github-mcp) as an alternative (note: can be token-expensive)
- Configure [Slack MCP](https://smart-ai.dev/slack/slack-mcp) for alerts
...
- [GitHub MCP on Smart AI Mission Control](https://smart-ai.dev/github/github-mcp) (alternative option)
