---
title: "Content Management with Sanity MCP and Smart AI"
...
sidebarTitle: "Sanity CMS with Smart AI"
---
...
<Card title="What You'll Build" icon="database">
  An AI-powered content management system workflow that uses Smart AI's AI agent with Sanity
...
<Step title="Install Sanity CLI">
 ...
 <Warning>
  To use agents in headless mode, you need a [Smart AI API key](https://smart-ai.dev/settings/api-keys) and proper environment variable configuration.
  </Warning>
  ...
  <Tab title="⚡ Quick Start (Recommended)">
    <Steps>
      <Step title="Load the Pre-Built Agent">
        Visit the [Sanity Assistant Agent](https://smart-ai.dev/continuedev/sanity-assistant-agent) on Smart AI Mission Control and click **"Install Config"** or run:
        ...
        This agent includes:
        - **[Sanity MCP](https://smart-ai.dev/sanity/sanity-mcp)** pre-configured and ready to use
...
<Steps>
  ...
  <Step title="Connect Sanity MCP via Smart AI Mission Control">
    Visit the [Sanity MCP on Smart AI Mission Control](https://smart-ai.dev/sanity/sanity-mcp) and click **"Install"** to add it to the agent you created in the step above.
    ...
    <Tip>
      **Alternative installation methods:**
      ...
      2. **With config**: Use [Sanity MCP Config](https://smart-ai.dev/sanity/sanity-mcp-config) for environment variable setup
      3. **Manual configuration**: Add the MCP to your agent configuration

      Once installed, Sanity MCP tools become available to your Smart AI agent for all prompts.
    </Tip>
    ...
    <Info>
      **Authentication Options:**
      ...
      See [Sanity MCP Config](https://smart-ai.dev/sanity/sanity-mcp-config) for environment variable setup.
    </Info>
...
<Accordion title="Agent Requirements">
    To use the pre-built Sanity Assistant agent, you need either:
      ...
      - **Your own API keys** added to Smart AI Mission Control secrets
...
Now you can use natural language prompts to manage your Sanity content and schemas. The Smart AI agent automatically calls the appropriate Sanity MCP tools.
...
<Info>
  The workflow uses the [Sanity Assistant Agent](https://smart-ai.dev/continuedev/sanity-assistant-agent) with environment variable authentication via [Sanity MCP Config](https://smart-ai.dev/sanity/sanity-mcp-config). This enables headless mode operation without OAuth browser authentication.
</Info>

### Create Workflow File

This workflow automatically validates your Sanity schemas and content on pull requests using the Smart AI CLI in [headless mode](/cli/headless-mode). It checks schema integrity, validates content relationships, and posts a summary report as a PR comment.
...
<Info>
  Environment variables enable the MCP to authenticate without OAuth browser prompts. The [Sanity MCP Config](https://smart-ai.dev/sanity/sanity-mcp-config) documentation provides detailed setup instructions for all required variables.
</Info>

## Content Management Best Practices

Implement automated content quality checks using Smart AI's rule system. See the [Rules deep dive](/customize/deep-dives/rules) for authoring tips.
...
<Check>
  **Verification Steps:**
  - Sanity MCP is installed via [Smart AI Mission Control](https://smart-ai.dev/sanity/sanity-mcp)
