---
title: "Automated Security Scanning with Snyk MCP and Smart AI"
...
sidebarTitle: "Snyk Security Scanning with Smart AI"
---
...
<Card title="What You'll Build" icon="shield-halved">
  An automated security scanning system that uses Smart AI's AI agent with Snyk
...
<iframe
  ...
  title="Snyk MCP Smart AI Cookbook Demo"
  frameborder="0"
...
<Note>
  **Important**: The Snyk MCP requires the Snyk CLI to be authenticated locally. Run `snyk auth` to authenticate before using the Smart AI agent with Snyk MCP.
</Note>
 <Warning>
...
<Steps>
  ...
  <Step title="Connect Snyk MCP via Smart AI Mission Control">
    Visit the [Snyk Continuous AI Agent](https://smart-ai.dev/continuedev/snyk-continuous-ai-agent) and click **Install** to add it to the agent you created in the step above.
    ...
    <Tip>
      **Alternative installation methods:**
      ...
      Once installed, Snyk MCP tools become available to your Smart AI agent for all prompts.
    </Tip>

    <Info>
      The MCP will request authentication and folder trust permissions when first used.
      This is handled automatically by the Smart AI agent.
    </Info>

  </Step>

  <Step title="Add Secure-at-Inception Rules">
    Install the [Snyk Secure-at-Inception rules](https://smart-ai.dev/snyk/secure-at-inception) from Mission Control to enable automatic security scanning.
    ...
    **How to add rules to your agent:**
    ...
    3. Rules apply globally to all your Smart AI sessions
...
Now you can use natural language prompts to run comprehensive security scans. The Smart AI agent automatically calls the appropriate Snyk MCP tools.
    ...
    steps:
      ...
      - name: Validate Secrets
        run: |
          ...
          if [ -z "${{ secrets.SMARTAI_API_KEY }}" ]; then
            echo "⚠️ Warning: SMARTAI_API_KEY not set - AI mitigation suggestions will be skipped"
          fi
      ...
      - name: Generate AI Mitigation Suggestions
        ...
        run: |
          ...
          # Create a summary of findings for Smart AI CLI
          ...
          if [ -n "$FINDINGS_SUMMARY" ]; then
            ...
            # Use Smart AI CLI to generate mitigation suggestions
      ...
      - name: Post Mitigation Summary to PR
        ...
        run: |
          ...
          **Scan Details:**
          ...
          - 🤖 Generated with Smart AI CLI + Snyk
...
<Info>
  **About SNYK_TOKEN**: The workflow uses the SNYK_TOKEN in two ways:
  ...
  2. **Smart AI CLI access** - Available as an environment variable when Smart AI generates AI mitigation suggestions
...
<Info>
  ...
  - **AI Mitigation**: Uses Smart AI CLI to generate actionable mitigation steps
...
Implement automated security policies using Smart AI's rule system. See the [Rules deep dive](/customize/deep-dives/rules) for authoring tips.

<Note>
  **Coming Soon**: These security guardrail prompts will be available as pre-configured rules on the Smart AI Mission Control for easy installation.
</Note>
...
<Check>
  **Verification Steps:** - Snyk MCP is installed via [Smart AI
  Hub](https://smart-ai.dev/snyk/snyk-mcp) - Secure-at-Inception rules are
  [enabled](https://smart-ai.dev/snyk/secure-at-inception) - Authentication
