---
title: "Using Ollama with Smart AI: A Developer's Guide"
description: "Complete guide to setting up Ollama with Smart AI for local AI development. Learn installation, configuration, model selection, performance optimization, and troubleshooting for privacy-focused offline coding assistance"
---
...
## How to Configure Ollama with Smart AI

There are multiple ways to configure Ollama models in Smart AI:

### Method 1: Using Hub Model Blocks in Local config.yaml

The easiest way is to use [pre-configured model blocks](/reference#models) from the Smart AI Mission Control in your local configuration:

```yaml title="~/.smart-ai/configs/config.yaml"
...
<Warning>
  **Important**: Blocks only provide configuration - you still need to pull
  the model locally. The block `ollama/deepseek-r1-32b` configures Smart AI
  to use `model: deepseek-r1:32b`, but the actual model must be installed:
  ```bash
  # Check what the block expects (view on smart-ai.dev)
...
Smart AI can automatically detect available Ollama models. You can configure this in your YAML:

```yaml title="~/.smart-ai/config.yaml"
models:
  - name: Autodetect
    provider: ollama
    model: AUTODETECT
...
3. Smart AI will scan for available Ollama models
...
<Note>
  ...
  available models. When set to `AUTODETECT`, Smart AI will dynamically populate
...
Smart AI may use a higher default context length than other tools. Reduce `contextLength` in your config (e.g., to 2048), or try a smaller model. See [Ollama provider troubleshooting](/customize/model-providers/top-level/ollama#troubleshooting) for details.
...
async def create_user(user: User):
    # Smart AI will help complete this implementation
...
Ollama with Smart AI provides a powerful local development environment for AI-assisted coding. You now have complete control over your AI models, ensuring privacy and enabling offline development workflows.

---

_This guide is based on Ollama v0.11.x and Smart AI v1.1.x. Please check for updates regularly._
