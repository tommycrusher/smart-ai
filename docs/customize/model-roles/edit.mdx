...
In Smart AI, you can add `edit` to a model's roles to specify that it can be used for Edit requests. If no edit models are specified, the selected `chat` model is used.
...
Explore edit models in [the hub](https://smart-ai.dev/explore/models?roles=edit). Generally, our recommendations for Edit overlap with recommendations for Chat.
...
You can customize the prompt template used for editing code by setting the `promptTemplates.edit` property in your model configuration. Smart AI uses [Handlebars syntax](https://handlebarsjs.com/guide/) for templating.
