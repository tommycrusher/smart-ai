---
title: "How to Configure Smart AI"
description: Learn how to access and manage Smart AI configurations through Hub or local YAML files
keywords: [config, settings, customize]
sidebarTitle: "Configuration"
---

You can easily access your configuration from the Smart AI Chat sidebar. Open the sidebar by pressing <kbd>cmd/ctrl</kbd> + <kbd>L</kbd> (VS Code) or <kbd>cmd/ctrl</kbd> + <kbd>J</kbd> (JetBrains) and click the Agent selector above the main chat input. Then, you can hover over an agent and click the `new window` (hub agents) or `gear` (local agents) icon.

![configure](/images/configure-continue.png)

## How to Manage Hub Configs

  Hub Configs can be managed in [the Hub](https://smart-ai.dev). See [Editing a config](/mission-control/configs/edit-a-config)
...
- `~/.smart-ai/config.yaml` (MacOS / Linux)
...
When editing this file, you can see the available options suggested as you type, or check the reference below. When you save a config file from the IDE, Smart AI will automatically refresh to take into account your changes. A config file is automatically created the first time you use Smart AI, and always automatically generated with default values if it doesn't exist.
...
- `config.ts` - Advanced configuration (probably unnecessary) - a TypeScript file in your home directory that can be used to programmatically modify (_merged_) the `config.json` schema:
  - `~/.smart-ai/config.ts` (MacOS / Linux)
...
`config.yaml` or `config.json` can handle the vast majority of necessary configuration, so we recommend using it whenever possible. However, if you need to programmatically extend Smart AI configuration, you can use a `config.ts` file, placed at `~/.smart-ai/config.ts` (MacOS / Linux) or `%USERPROFILE%\.continue\config.ts` (Windows).
