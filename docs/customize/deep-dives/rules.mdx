---
title: "How to Create and Manage Rules in Smart AI"
...
Smart AI supports two types of rules with different behaviors:

<Columns cols={2}>
<Card title="Local Rules" icon="folder">
- **Location**: `.smart-ai/rules` folder in your workspace
...
- **Location**: Stored on Smart AI Mission Control, referenced in config.yaml
...
When using Smart AI, rules are loaded in this order:
1. **Hub assistant rules** (if using a Hub-based assistant)
2. **Referenced Hub rules** (via `uses:` in config.yaml)
3. **Local workspace rules** (from `.smart-ai/rules` folder)
4. **Global rules** (from `~/.smart-ai/rules` folder)
...
1. Create a folder called `.smart-ai/rules` at the top level of your workspace
2. Add a file called `pirates-rule.md` to this folder.
3. Write the following contents to `pirates-rule.md` and save.

```md title=".smart-ai/rules/pirates-rule.md"
---
name: Pirate rule
---
...
For example, you can say "Create a rule for this", and a rule will be created for you in `.smart-ai/rules` based on your conversation.
</Info>

### Creating Hub Rules

Rules can also be created and managed on the Smart AI Mission Control:

1. **Browse existing rules**: [Explore available rules](https://smart-ai.dev/hub?type=rules)
2. **Create new rules**: [Create your own](https://smart-ai.dev/new?type=block&blockType=rules) in the Hub
3. **Copy from local rules**: Copy/paste content from your `.smart-ai/rules` files to create Hub rules
...
1. Copy the content from your `.smart-ai/rules/rule-name.md` file
2. Go to [Create new rule](https://smart-ai.dev/new?type=block&blockType=rules)
...
globs: docs/**/*.{md,mdx}
alwaysApply: false
description: Standards for writing and maintaining Smart AI Docs
---

# Smart AI Docs Standards
...
You can create project-specific rules by adding a `.smart-ai/rules` folder to the root of your project and adding new rule files.
...
```md title=".smart-ai/rules/typescript.md"
---
...
**Root Cause**: This happens when you have a mix of local and Hub rules, and Smart AI can't properly determine where each rule originates.

**Workaround**:
1. **For local rules**: Navigate directly to `.smart-ai/rules/` folder and edit the `.md` file
2. **For Hub rules**: Go directly to your assistant page on [Smart AI Mission Control](https://smart-ai.dev) and edit from there
...
**Check These**:
1. **File location**: Ensure local rules are in `.smart-ai/rules/` (not `.smart-ai/rule/`)
...
Smart AI includes a simple default system message for [Agent mode](../../ide-extensions/agent/quick-start) and [Chat](../../ide-extensions/chat/quick-start) requests, to help the model provide reliable codeblock formats in its output.
