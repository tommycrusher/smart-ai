---
title: "How to Set Up Model Context Protocol (MCP) in Smart AI"
...
[here](https://smart-ai.dev/explore/mcp).
...
```yaml title=".smart-ai/mcpServers/playwright-mcp.yaml"
...
## How to Set Up Smart AI Documentation Search with MCP

You can set up an MCP server to search the Smart AI documentation directly from your config. This is particularly useful for getting help with Smart AI configuration and features.

For complete setup instructions, troubleshooting, and usage examples, see the [Smart AI MCP Reference](/reference/continue-mcp).

## Using JSON MCP Format from Claude, Cursor, Cline, etc

<Info>
If you're coming from another tool that uses JSON MCP format configuration files (like Claude Desktop, Cursor, or Cline), you can copy those JSON config files directly into your `.smart-ai/mcpServers/` directory (note the plural "Servers") and Smart AI will automatically pick them up.

For example, place your JSON MCP config file at `.smart-ai/mcpServers/mcp.json` in your workspace.
</Info>
...
`mcpServers`](https://smart-ai.dev/new?type=block&blockType=mcpServers) or add a local MCP
...
<Note>
When creating a standalone block file in `.smart-ai/mcpServers/`, remember to include the required metadata fields (`name`, `version`, `schema`) as shown in the Quick Start example above.
</Note>
...
as well as access hosted secrets in the Smart AI Mission Control. To leverage Hub secrets, you can use the `inputs` property in your MCP env block instead of `secrets`.
