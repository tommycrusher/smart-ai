---
title: "How to Configure ClawRouter with Smart AI"
...
Add multiple ClawRouter profiles to your config and switch via Smart AI's model picker:
...
Use the **model picker dropdown** in Smart AI's chat panel to switch between profiles. Each profile routes to different model tiers based on cost vs. quality trade-offs.

<Tip>
  **Quick switch via CLI:** In the Smart AI chat, type `/model` followed by the profile name (e.g., `/model ClawRouter Eco`).
</Tip>
...
You can configure ClawRouter for different Smart AI roles:
...
ClawRouter manages upstream provider API keys internally. You configure them in ClawRouter, not in Smart AI.
...
| Error | Smart AI's Default | ClawRouter's Handling |
|-------|-------------------|----------------------|
