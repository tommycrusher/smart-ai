...
The `@Docs` context provider allows you to efficiently reference documentation directly within Smart AI.
...
By default, Smart AI crawls documentation sites using a specialized crawling service that provides the best experience for most users and documentation sites.
...
<Info>
  ...
  to `~/.smart-ai/.utils`, and use it as the local crawler. **Note:** Chromium
...
- Smart AI does not automatically re-index your docs. Use `Click to re-index` to trigger a reindex for a specific source
...
You can also use the following IDE command to force a re-index of all docs: `Smart AI: Docs Force Re-Index`.
