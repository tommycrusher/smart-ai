...
<Warning>
  **This page documents deprecated functionality.** For creating slash commands and customizing Smart AI, please use:
...
Below are details for each property that can be set in `config.json`. The config schema code is found in [`extensions/vscode/config_schema.json`](https://github.com/tommycrusher/smart-ai/blob/main/extensions/vscode/config_schema.json).
...
```
"docs": [    {    "title": "Smart AI",    "startUrl": "https://docs.smart-ai.dev/intro",    "faviconUrl": "https://docs.smart-ai.dev/favicon.ico",  }]
```
  ...
  "slashCommands": [
    {
      ...
      "params": { "outputDir": "~/.smart-ai/session-transcripts" }
    }
  ]
}
```
...
Write a custom slash command at your own HTTP endpoint. Set 'url' in the params object for the endpoint you have setup. The endpoint should return a sequence of string updates, which will be streamed to the Smart AI sidebar. See our basic [FastAPI example](https://github.com/tommycrusher/smart-ai/blob/main/core/context/providers/context_provider_server.py#L34-L45) for reference.
...
Describe the issue you'd like to generate, and Smart AI will turn into a well-formatted title and body, then give you a link to the draft so you can submit. Make sure to set the URL of the repository you want to generate issues for.
  ...
  "slashCommands": [
    {
      ...
      "params": { "repositoryUrl": "https://github.com/tommycrusher/smart-ai" }
    }
  ]
}
```
