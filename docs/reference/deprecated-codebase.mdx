...
Smart AI respects `.gitignore` files in order to determine which files should not be indexed. If you'd like to exclude additional files, you can add them to a `.smartaiignore` file, which follows the exact same rules as `.gitignore`.

Smart AI also supports a **global** `.smartaiignore` file that will be respected for all workspaces, which can be created at `~/.smart-ai/.smartaiignore`.
