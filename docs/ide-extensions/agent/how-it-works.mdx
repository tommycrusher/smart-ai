...
4. Smart AI calls the tool using built-in functionality or the MCP server that offers that particular tool
5. Smart AI sends the result back to the model
...
Smart AI includes several built-in tools which provide the model access to IDE functionality.
...
- **Create Rule Block** (`create_rule_block`): Create a new rule block in `.smart-ai/rules`
