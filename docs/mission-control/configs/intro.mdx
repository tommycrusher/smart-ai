...
<Info>
  **New UI Update: The Hub is now Mission Control.**
  If you're returning after the redesign, you can now find and manage configs in your [Mission Control Settings](https://smart-ai.dev/settings/configs).
</Info>

## What Are Configs?

Configs are flexible containers that combine multiple components to create powerful AI coding experiences using the Smart AI CLI or the Extensions. A single config can include:
...
<Steps>
  ...
  <Step title="Explore existing configs">
    ...
    Browse the [Smart AI Mission Control](https://smart-ai.dev/hub?type=configs) to see what configs others have built
  ...
  <Step title="Use in your IDE">
    ...
    Select the config from the dropdown in your Smart AI extension, type `/config` in the CLI, or use `--config` with the CLI commands
