...
Check files live in `.smart-ai/checks/` or `.agents/checks/` at the repository root.

## Frontmatter fields

| Field | Required | Type | Description |
|-------|----------|------|-------------|
| `name` | Yes | string | Display name shown in GitHub status checks and on smart-ai.dev |
...
```markdown .smart-ai/checks/migration-safety.md
---
name: Migration Safety
description: Flag destructive database migrations
...
Smart AI locates checks by scanning specific directories at the repository root:

- Reads all `.md` files in `.smart-ai/checks/`
...
`.smart-ai/checks/` and `.agents/checks/` are equivalent. Both are scanned and merged into a single list of checks.

`.agents/` is an emerging cross-tool standard for AI agent configuration. `.smart-ai/` is Smart AI-specific. Use whichever fits your project. If both directories contain a check with the same `name`, the `.smart-ai/checks/` version takes precedence.
