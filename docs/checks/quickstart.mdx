...
```
Help me write checks for this codebase: https://smart-ai.dev/walkthrough
```

The walkthrough reads your codebase, helps you write your first checks, connects you to Smart AI, and gets them running on your next PR. It takes a few minutes.

If you'd rather set things up manually, keep reading.

## Writing checks manually

A check file has two parts: **frontmatter** (YAML between `---` delimiters) with a required `name` and `description`, and a **body** prompt that tells the AI what to look for. Create a `.md` file in `.smart-ai/checks/`:
