# Contributing to Smart AI (JetBrains extension) <!-- omit in toc -->
...
Smart AI is built with JDK version 17 (as specified in [`./build.gradle.kts`](./build.gradle.kts)), which can be
...
Select the `Run Smart AI` task in the top right corner of the IDE and then select the "Debug" option.

> In community edition, use `Run Smart AI (CE)` instead, which uses shell scripts instead of Ultimate-only node configs.
...
When running the `Start Core Dev Server` task, we set the location of your Smart AI directory to
`./extensions/.smart-ai-debug`. This is to
allow for changes to your `config.json` and other files during development, without affecting your actual configuration.

### Viewing logs

When using the `Run Smart AI` task, we automatically tail both prompt logs and IDE logs.
