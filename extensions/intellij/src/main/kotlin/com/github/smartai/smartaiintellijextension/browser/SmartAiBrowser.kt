...
class SmartAiBrowser(
    private val project: Project,
    private val gsonService: GsonService = service<GsonService>(),
): Disposable {

    private val log = Logger.getInstance(SmartAiBrowser::class.java.simpleName)
    ...
    init {
        ...
        myJSQueryOpenInBrowser.addHandler { msg: String? ->
            ...
            if (MessageTypes.PASS_THROUGH_TO_CORE.contains(messageType)) {
                project.service<SmartAiPluginService>().coreMessenger?.request(messageType, data, messageId) { data ->
            ...
            if (msg != null) {
                project.service<SmartAiPluginService>().ideProtocolClient?.handleMessage(msg) { data ->
                    sendToWebview(
                        messageType,
                        mapOf(
                            "status" to "success",
        ...
        project.service<SmartAiPluginService>().onProtocolClientInitialized {
