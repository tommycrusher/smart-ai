...
@Service
class SmartAiAuthService {
    private val coroutineScope = CoroutineScope(Dispatchers.IO)
    private val log = Logger.getInstance(SmartAiAuthService::class.java)
    ...
    fun startAuthFlow(project: Project, useOnboarding: Boolean) {
        ...
        ApplicationManager.getApplication().invokeLater {
            val dialog = SmartAiAuthDialog(useOnboarding, url) { token ->
                // Store the token
    ...
    private fun updateRefreshToken(token: String) {
        try {
            ...
            service<SmartAiSentryService>().report(e, "Exception while refreshing token ${e.message}")
        }
    }
