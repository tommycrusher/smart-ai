...
describe("TipsDisplay", () => {
  ...
  it("renders a random tip from the tips array", () => {
    ...
    const expectedTipIndex = Math.floor(0.5 * SMARTAI_CLI_TIPS.length);
    const expectedTip = SMARTAI_CLI_TIPS[expectedTipIndex];
  ...
  it("always renders the tip structure correctly", () => {
    ...
    const containsAtLeastOneTip = SMARTAI_CLI_TIPS.some(
...
describe("SMARTAI_CLI_TIPS", () => {
  it("contains at least 6 tips", () => {
    expect(SMARTAI_CLI_TIPS.length).toBeGreaterThanOrEqual(6);
  });

  it("all tips are non-empty strings", () => {
    SMARTAI_CLI_TIPS.forEach((tip, index) => {
  ...
  it("tips are helpful and informative", () => {
    ...
    const allTipsText = SMARTAI_CLI_TIPS.join(" ").toLowerCase();
