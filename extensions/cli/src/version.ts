  ...
  latestVersionCache = (async () => {
    try {
      ...
      const response = await fetch(
        `https://api.smart-ai.dev/cn/info?id=${encodeURIComponent(id)}`,
        { signal },
      );
      if (!response.ok) {
      ...
      logger?.debug(
        "Warning: Could not fetch latest version from api.smart-ai.dev",
      );
      return null;
    }
  })();
...
getLatestVersion()
  ...
  .catch((error) => {
    logger?.debug(
      `Warning: Could not fetch latest version from api.smart-ai.dev: ${error}`,
    );
  });
