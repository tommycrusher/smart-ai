...
describe("PosthogService Error Handling", () => {
  ...
  it("should not capture when service is disabled", async () => {
    ...
    const originalEnv = process.env.SMARTAI_ALLOW_ANONYMOUS_TELEMETRY;
    process.env.SMARTAI_ALLOW_ANONYMOUS_TELEMETRY = "0";
    ...
    if (originalEnv === undefined) {
      delete process.env.SMARTAI_ALLOW_ANONYMOUS_TELEMETRY;
    } else {
      process.env.SMARTAI_ALLOW_ANONYMOUS_TELEMETRY = originalEnv;
    }
  });
});
