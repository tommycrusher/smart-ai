...
- **Environment persistence:** The devbox entrypoint (control-plane) writes all env vars to `~/.smart-ai/devbox-env` and sources it before `cn serve`, so keys survive suspend/resume. The CLI assumes env is already present.

Operational notes:

- Changing the entrypoint is expensive; prefer adapting CLI/session behavior as above.
- When testing suspend/resume, confirm a single session file under `~/.smart-ai/sessions` for the agent id and that follow-up messages append normally without replaying the first prompt.
