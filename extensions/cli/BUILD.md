...
The Smart AI CLI uses esbuild to bundle the application along with local packages (`@smartai/config-yaml` and `@smartai/openai-adapters`) into a single distributable file. This ensures that users who install the CLI from npm don't need to worry about local file references.
...
- Include local packages (`@smartai/config-yaml`, `@smartai/openai-adapters`) directly in the bundle
...
2. Users install via `npm install -g @smartai/cli`
