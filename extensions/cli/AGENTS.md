...
This is a CLI tool for Smart AI Dev that provides an interactive AI-assisted development experience. The architecture consists of:
...
3. **Smart AI SDK Integration** (`src/continueSDK.ts`): Initializes the Smart AI SDK client with:
...
7. **Hooks System** (`src/hooks/`): Event interception system for extending CLI behavior
   ...
   - **Config locations** (lowest to highest precedence):
     - `~/.claude/settings.json`, `~/.smart-ai/settings.json` (user-global)
     - `.claude/settings.json`, `.smart-ai/settings.json` (project)
     - `.claude/settings.local.json`, `.smart-ai/settings.local.json` (project-local)
